...
- This is why they remain unchanged, whatever the model used. They are **featureless baselines** because they don't use node attributes at all.
...
**Feature ablation**:
By removing or restricting node attributes (Full features → Numerical only → None).
→ Evaluates the contribution of semantic information.

**Model ablation**:
By replacing GCN layers with a Linear encoder.
→ Tests whether message passing truly helps beyond a simple linear projection.

**Depth ablation**:
By reducing the number of GCN layers from 2 to 1.
→ Checks if deeper architectures are beneficial or redundant for this topology.

#### b. Ablation study table
...
- the 2-layers GCN model outperforms the 1-layer one
...
4. **Abaltion study** allows to validate a clean, minimalist model design which only includes features proven to be beneficial to model efficiency.
